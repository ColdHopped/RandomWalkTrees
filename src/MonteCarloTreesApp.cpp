...
class MonteCarloTreesApp : public App {  // NOLINT(cert-msc51-cpp)
  public:
	void setup() override;
...
void MonteCarloTreesApp::setup()
{
	...
	mCamera.lookAt(vec3(2.5, 2.5, 5), vec3(0, 0, 0), vec3(0, -1, 0));
	...
	walkPath.resize(numberOfSteps + 1);

	// Generate initial points
	mStartPoints = generatePoints(2, -1, 1, -1, 1, -1, 1);
...
void MonteCarloTreesApp::update()
{
	...
	if (currentStep < walkPath.size())
	{
		updating = true;
		if (currentStep == 0)
		{
			walkPath[currentStep] = mStartPoints[0];
		}
		else
		{
			walkPath[currentStep] = generateRandomWalk(walkPath[currentStep - 1].x, walkPath[currentStep - 1].y, walkPath[currentStep - 1].z, 0.2);
		}
...
{
	// Seed the RNG
	std::mt19937 rng(static_cast<unsigned int>(std::time(nullptr)));  // NOLINT(cert-msc51-cpp)
