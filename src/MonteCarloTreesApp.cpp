...
private:
	...
	static vec3 generateRandomWalkCircle(float startX, float startY, float startZ, float maxR = 1, float maxPhi = M_PI, float maxTheta = 2 * M_PI);
		...
		else
		{
			for(int index = 0; index < numberOfStartingPoints; index++)
			{
				...
				if(currentStep < 2)
				{
					...
					walkPath[index][currentStep] = generateRandomWalkCircle(previousPoint.x, previousPoint.y, previousPoint.z, 0.1);
				}
				else
				{
...
vec3 MonteCarloTreesApp::generateRandomWalkCartesian(float startX, float startY, float startZ, float maxX, float maxY, float maxZ)
{
	// Input checks
	if (maxX < 0) {
		CI_LOG_E("Error: maxX should not be smaller than 0. Setting maxX to 0.");
		maxX = 0;
	}
	if (maxY < 0) {
		CI_LOG_E("Error: maxY should not be smaller than 0. Setting maxY to 0.");
		maxY = 0;
	}
	if (maxZ < 0) {
		CI_LOG_E("Error: maxZ should not be smaller than 0. Setting maxZ to 0.");
		maxZ = 0;
	}
	...
	// Define distributions for each coordinate
	std::uniform_real_distribution<float> rDistribution(0, maxX);
	std::uniform_real_distribution<float> phiDistribution(0, maxY);
	std::uniform_real_distribution<float> thetaDistribution(0, maxZ);

	// Generate random walk
	float x = rDistribution(rng);
	float y = phiDistribution(rng);
	float z = thetaDistribution(rng);
...
vec3 MonteCarloTreesApp::generateWeightedRandomWalk(float x, float y, float z)
{
	...
	vec3 randomWalk = generateRandomWalkCircle(x, y, z, r, phi, 2 * M_PI);
